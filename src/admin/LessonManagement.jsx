...
export default function LessonManagement({ isMobile, isTablet }) {
  ...
  const resetForm = () => {
    setForm({ lesson_name: '', level: '', status: 'Active' })
    setFormError('')
    setFormSuccess('')
    setEditingId(null)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.lesson_name || !form.level) {
      setFormError('Lesson name and level are required.')
      return
    }
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: C.mist }}>
                  {['Lesson', 'Level', 'Status', 'Action'].map(h => (
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map(l => (
                    <tr key={l.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <td style={{ padding: '12px 14px', fontSize: '0.8rem', color: C.text2 }}>{l.level}</td>
